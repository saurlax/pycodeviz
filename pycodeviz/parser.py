...
class ProjectAnalyzer:
    ...
    def _matches_patterns(self, file_path: Path, patterns: List[str]) -> bool:
        """Check if a file matches any of the given glob/fnmatch patterns"""
        try:
            relative = str(file_path.relative_to(self.project_path))
        except ValueError:
    ...
    def _analyze_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            logger.warning("Failed to parse %s: %s", file_path, error_msg)
            logger.debug("Full traceback:\n%s", traceback.format_exc())
        except UnicodeDecodeError as e:
            error_msg = f"Encoding error: {e}"
            self.failed_files.append((str(file_path), error_msg))
            logger.warning("Failed to read %s: %s", file_path, error_msg)
        except Exception as e:
            error_msg = str(e)
            self.failed_files.append((str(file_path), error_msg))
            logger.warning("Failed to analyze %s: %s", file_path, error_msg)
            logger.debug("Full traceback:\n%s", traceback.format_exc())
    ...
    def _should_skip(self, file_path: Path) -> bool:
        """Check if file should be skipped based on built-in rules and user patterns"""
        ...
        if self.include_patterns and not self._matches_patterns(file_path, self.include_patterns):
            logger.debug("Skipping %s (not matching --include patterns)", file_path)
            return True

        # If exclude patterns are specified, skip matching files
        if self.exclude_patterns and self._matches_patterns(file_path, self.exclude_patterns):
            logger.debug("Skipping %s (matching --exclude pattern)", file_path)
            return True
